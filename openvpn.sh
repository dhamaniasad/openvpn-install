...
if [ -e /etc/openvpn/server.conf ]; then
	...
	clear
		...
		case $option in
			...
			;;
			3) 
			yum -y remove openvpn openvpn-blacklist
			rm -rf /etc/openvpn
    ...
    wget --no-check-certificate https://github.com/dhamaniasad/openvpn-install/raw/master/rpm/openvpn-2.3.2-2.el6.i686.rpm
	yum -y install openvpn-2.3.2-2.el6.i686.rpm
	yum -y install iptables openssl
	...
	cp /usr/share/doc/openvpn-2.3.2/sample/sample-config-files/client.conf ~/ovpn-$CLIENT/$CLIENT.conf
	cp /etc/openvpn/easy-rsa/2.0/keys/ca.crt ~/ovpn-$CLIENT
	cp /etc/openvpn/easy-rsa/2.0/keys/$CLIENT.crt ~/ovpn-$CLIENT
	cp /etc/openvpn/easy-rsa/2.0/keys/$CLIENT.key ~/ovpn-$CLIENT
	cd ~/ovpn-$CLIENT
